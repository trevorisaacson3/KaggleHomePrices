...
for(i in mino) {
  model_gbm <- gbm(SalePrice ~ ., distribution = "gaussian", data = training, cv.folds = 7, shrinkage = .1, 
...
model_final_bag <- gbm(SalePrice ~ ., distribution = "gaussian", data = trainData, cv.folds = 7, shrinkage = .1, 
...
MSEnumbers = c(linear_MSE, df_X2ndFlrSF[1,2], gam_MSE, tree_MSE, min(rmse), rf_MSE)
