...
model_final_bag <- gbm(SalePrice ~ ., data = trainData, distribution = 'gaussian', cv.folds = 7, shrinkage = .1, 
                   n.minobsinnode = which(bag_rmse == min(bag_rmse)), n.trees = nrow(trainData), 
                   verbose = FALSE)
...
```{r}
method = c("Linear Regression", "PCR", "PLS", "Splines", "GAM", "Trees", "Bagging", "Random Forest")
MSEnumbers = c(linear_MSE, 0, 0, df_X2ndFlrSF[1,2], gam_MSE, tree_MSE, bag_rmse, rf_MSE)
...
After running all of our models we thought bagging would have proven to be our best bet for placement in the competition. It yielded our lowest rmse value using our training and testing data at `r min(rmse)`. However, that proved to be misleading as the submission with the actual test data wasn't that close.
